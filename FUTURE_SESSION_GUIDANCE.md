...
The coordinator uses the lightweight `/campaigns/:id/render-assets` route rather than loading the full Studio workspace. Before publishing, use **Run Editor assessment** to call `Automation.review_batch/2`. The Editor reads every generated slide and all six channel drafts, then persists its approve/revise/reject verdict, overall score, thematic-consistency score, interest score, shareability score, strengths, concerns, and recommendations in the plan's `selection_details["editor_review"]`. Treat a `revise` or `reject` result as editorial evidence to inspect; rerunning the Editor safely replaces the prior assessment.

Normal coordinator preparation calls `Automation.prepare_quality_batch/2`. It generates the canonical package, runs the Editor, and accepts it only when the verdict is `approve` and the overall, consistency, interest, and shareability scores all meet the default threshold of 75. A weaker package receives at most one feedback-guided, source-grounded story revision, is regenerated, and is scored again. Reviews are keyed to the canonical generated asset IDs, so subsequent browser resumes do not repeat model calls. The revision count and prior review are retained in `quality_revision_count` and `quality_history`. Never bypass the hard revision limit, and never revise a plan whose canonical drafts are already scheduled or published.
