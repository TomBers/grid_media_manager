defmodule GridMediaManager.ApplicationTest do
  ...
  test "summarizes launch environment without exposing secrets" do
    ...
    assert summary =~ "AWS_ACCESS_KEY_ID=set (12 chars)"
    assert summary =~ "AWS_SECRET_ACCESS_KEY=set (15 chars)"
    assert summary =~ "RATIONALGRID_PROMOTION_API_TOKEN=set (20 chars)"
    assert summary =~ "BUFFER_API_KEY=set (18 chars)"
    assert summary =~ "PEXELS_API_KEY=set (18 chars)"
    refute summary =~ "ending"
